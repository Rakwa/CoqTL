...
Definition Rule_incl_guard' {tc: TransformationConfiguration} (x y: Rule) : Prop :=
  Rule_getName x = Rule_getName y
  /\ (Rule_getGuardExpr x = Rule_getGuardExpr y  \/
      Rule_getGuardExpr x = (fun _ _ => None))
...
Definition Transformation_incl_elements''' {tc: TransformationConfiguration} (t1 t2: Transformation) : Prop :=
  ...
  (forall (r1: Rule), In r1 (Transformation_getRules t1) -> 
    (exists r2:Rule, In r2 (Transformation_getRules t2) /\ Rule_incl_guard' r1 r2 )). 

Theorem additivity_guard_general :
...
* apply in_flat_map.
  ...
  + destruct H6.
    ++ apply filter_In.
       ...
       rewrite <- H6. auto.
    ++ unfold matchRuleOnPattern,evalGuardExpr in H3.
       rewrite H6 in H3. simpl in H3. crush.
  + apply in_flat_map.
...
* intro.
  ...
  **  intros.
      ...
      - exists y.
        ...
        -- crush.
        -- rewrite <- H2.
           unfold Rule_incl_guard in H1.
           unfold Rule_incl_guard'; crush.
           induction H6.
           --- contradiction.
           --- destruct H5; crush.
           --- crush.
           --- induction H6.
                ...
                + simpl in H1.
                  ...
                  ++ simpl. right. crush.
                + simpl.
                  specialize (IHsubseq H1). right. auto.
...
Theorem additivity_guard_correct :
forall (tc: TransformationConfiguration) (t1 t2: Transformation) (sm: SourceModel),
  (Transformation_incl_elements'' t1 t2 -> 
    incl (allModelElements (execute t1 sm)) (allModelElements (execute t2 sm))).
Proof.
...
specialize (additivity_guard_general tc t1 t2).
auto.
Qed.
